...
import { loadRadialHeatmapPayload, readRadialHeatmapCache } from '../utils/radialHeatmapClient';
...
const ROTATE_NEWEST_WEEK_TO_TOP_DEG = (-((N_WEEKS - 1) * STRIDE_RAD * 180) / Math.PI);
...
function annularPath(cx: number, cy: number, ri: number, ro: number, a0: number, a1: number): string {
...
function monthLabels(fromSec: number, toSec: number): { a: number; text: string }[] {
...
function normalizeArtists(artists: RadialHeatmapPayload['artists']): RadialHeatmapPayload['artists'] {
...
export default memo(function RadialArtistHeatmap() {
    ...
    const stickyTouchTooltip =
        coarsePointer || (typeof navigator !== 'undefined' && navigator.maxTouchPoints > 0);
    ...
    const applyWeekHover = useCallback((week: number | null) => {
        const m = modelRef.current;
        if (!m) return;
        if (week === null) {
            flushHighlight();
    ...
    useEffect(() => {
        ...
        loadRadialHeatmapPayload()
            ...
            .catch((err: unknown) => {
                if (cancel) return;
                const message = err instanceof Error ? err.message : 'Failed to load yearly scrobbles';
                setError(message);
    ...
    const model = useMemo((): BuiltModel | null => {
        ...
        for (let r = 0; r < N_RINGS; r++) {
            const plays = artists[r]?.plays ?? Array.from({ length: N_WEEKS }, () => 0);
        ...
        const colors = artists.map((_, i) => ARTIST_PALETTE[i % ARTIST_PALETTE.length]);
    ...
    const setCellRef = useCallback((ring: number, week: number, el: SVGPathElement | null) => {
        if (!cellsRef.current[ring]) cellsRef.current[ring] = [];
        cellsRef.current[ring][week] = el;
    }, []);

    const pointerToChartSpace = useCallback((clientX: number, clientY: number): { x: number; y: number } | null => {
        const svg = svgRef.current;
        if (!svg) return null;
    ...
    const updateTooltip = useCallback(
        (week: number | null, clientX: number, clientY: number) => {
            ...
            for (let r = 0; r < N_RINGS; r++) {
                ...
                const plays = m.artists[r]?.plays?.[week] ?? 0;
                if (plays > 0) rows.push({ name, plays, color: m.colors[r] ?? m.colors[0] });
            }
    ...
    const onSvgMouseMove = useCallback(
        ...
        [applyWeekHover, coarsePointer, flushHighlight, pointerToChartSpace, updateTooltip]
    );
    ...
    const onSvgTouchComplete = useCallback(
        ...
        [applyWeekHover, flushHighlight, pointerToChartSpace, stickyTouchTooltip, updateTooltip]
    );
    ...
    return (
        <div
            ref={bindRoot}
            className={cn(
                'radial-heatmap-root relative flex w-full min-w-0 flex-1 flex-col gap-3 overflow-hidden pb-1',
                'min-h-[min(280px,85vw)] sm:min-h-[300px]'
            )}
        >
            <h2 id="radial-heatmap-heading" className="sr-only">
                Yearly scrobbles (week-wise): weekly listening intensity for your leading artists over the past year
            </h2>
            {showError ? (
                <div className="flex flex-1 items-center justify-center px-3 py-6 text-sm text-red-400">{error}</div>
            ) : null}
            {showSkeleton ? (
                <div className="radial-heatmap-shimmer flex flex-1 flex-col justify-center gap-2.5 py-5 sm:gap-3" aria-hidden="true">
                    <div
            ...
            {chartReady && model ? (
                <>
                    <div className="relative w-full max-w-full shrink-0">
                        ...
                        >
                            <desc>
                                Yearly scrobbles shown week by week: ten concentric rings for your most-played artists
                                over the last year. Each ring is split into fifty-two weeks; stronger segments mean more
                                plays that week for that artist.
                            </desc>
                            <g ref={chartGroupRef} transform={CHART_ROTATE_TRANSFORM}>
                                ...
                                <circle
                                    ...
                                    className={cn(
                                        'pointer-events-none',
                                        isLightTheme ? 'fill-blue-50/95' : 'fill-neutral-900/10 dark:fill-neutral-100/10'
                                    )}
                                />
                                {STATIC_PATHS.map((row, ring) =>
                                    row.map((d, week) => (
                                        <path
                                            ...
                                            fillOpacity={
                                                model.baseOpacities[ring]?.[week] ?? (isLightTheme ? 0.14 : 0.06)
                                            }
                                            stroke="none"
                                        />
                                    ))
                        ...
                        >
                            ...
                            <ul ref={tooltipListRef} className="m-0 max-h-48 list-none space-y-1 overflow-auto p-0" />
                        </div>
                    </div>
